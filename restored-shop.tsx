...
function ShopPage() {
  ...
  const filtered = useMemo(() => {
    ...
    else if (sort === "bestseller") result = result.filter((p) => p.isBestseller).concat(result.filter((p) => !p.isBestseller));
  ...
  const hasFilters = !!(query || selectedCategory || selectedOccasion || showInStockOnly || maxPrice < 60000);
      ...
      <div className="max-w-7xl mx-auto px-6 lg:px-10 py-8">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
          ...
          <div className="flex items-center gap-3 text-sm text-muted-foreground">
            ...
            {hasFilters && (
              <button onClick={clearFilters} className="text-gold text-xs hover:underline flex items-center gap-1">
                <X className="w-3 h-3" />
                Clear filters
              </button>
            )}
        ...
        <div className="flex flex-col sm:flex-row gap-3 mb-4">
          <div className="relative flex-1">
            ...
            />
            {query && (
              <button onClick={() => setQuery("")} className="absolute right-3 top-1/2 -translate-y-1/2 text-muted-foreground hover:text-gold">
                <X className="w-4 h-4" />
              </button>
            )}
          </div>
        ...
        {showFilters && (
          <div className="border border-border bg-card p-6 mb-6 grid sm:grid-cols-2 lg:grid-cols-4 gap-6">
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-2">
                {occasions.map((occ) => (
                  <button
                    key={occ.name}
                    onClick={() => setSelectedOccasion(occ.name === selectedOccasion ? "" : occ.name)}
            ...
            {/* Availability */}
            <div>
              <div className="text-[10px] tracking-luxury uppercase text-gold mb-3">Availability</div>
              <label className="flex items-center gap-2 cursor-pointer">
                ...
                >
                  <div className={`absolute top-0.5 w-4 h-4 bg-white rounded-full transition-transform shadow ${showInStockOnly ? "translate-x-5" : "translate-x-0.5"}`} />
                </div>
            ...
            <div className="flex items-end">
              <button onClick={clearFilters} className="text-xs tracking-luxury uppercase text-muted-foreground hover:text-gold transition-colors border border-border px-4 py-2">
                Reset All
              </button>
            </div>
          </div>
        ...
        {filtered.length === 0 ? (
          <div className="text-center py-24">
            ...
            <button onClick={clearFilters} className="btn-gold mt-6">Clear All Filters</button>
          </div>
        ) : (
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4 md:gap-6">
            {filtered.map((p) => {
              ...
              return (
                <div key={p.id} className="group reveal">
                  <div className="relative aspect-[3/4] overflow-hidden mb-3">
                    ...
                    {!p.inStock && (
                      <div className="absolute inset-0 bg-black/50 flex items-center justify-center pointer-events-none">
                        <span className="text-[10px] tracking-luxury uppercase text-white bg-black/70 px-3 py-1">Out of Stock</span>
                      </div>
                    )}
